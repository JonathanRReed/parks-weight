...
const KEY = "park-weight-onboarded";

const STEPS = [
  { title: "These are real park maps.", copy: "Every outline is the actual national park boundary, not a cartoon circle." },
  { title: "Tap a photo to pick a park.", copy: "Left is the bigger park. Right is the one stacked on top, at the real size." },
  { title: "North stays up.", copy: "Nothing is stretched to fill the screen. If a park looks small, it is small." },
];
...
function interesting(p, other) {
  ...
  const miles = Math.sqrt(p.acres / 640);
  const bits = [
    `${fmt(perAcre, 2)} visitors per acre`,
    `If it were a square, each side would be about ${fmt(miles, 1)} miles`,
  ];
  if (other && other.name !== p.name) {
    const times = other.acres / p.acres;
    bits.push(times >= 1 ? `${fmt(times, 1)} of these fit in ${other.name}` : `${fmt(1 / times, 1)}× bigger than ${other.name}`);
  }
  if (p.year === 1872) bits.push("The first U.S. national park");
  if (p.trip) bits.push("On this week’s Colorado–Utah road");
  return bits.join(" · ");
}
...
function showFacts(p, other) {
  if (!p) return;
  focus = p.name;
  $("#factKicker").textContent = p.trip ? `Rank #${p.rank} of 63 · this week` : `Rank #${p.rank} of 63`;
...
function draw() {
  ...
  $("#hint").textContent = "True size. North is up. Tap a map for its facts.";
...
function cardEl(p, onClick) {
  ...
  small.textContent = `${fmt(p.acres)} acres`;
  body.append(b, small);
  btn.append(pic, body);
...
function renderChooser(q = "") {
  ...
  if (!hits.length) {
    ...
    empty.textContent = "No parks by that name. Try “Zion” or “Denali”.";
    root.append(empty);
    return;
  }
...
function openChooser(role) {
  ...
  $("#chooserTitle").textContent = role === "big" ? "Pick the bigger park" : "Pick the park to stack on top";
  $("#search").value = "";
  renderChooser();
  $("#search").focus();
}
